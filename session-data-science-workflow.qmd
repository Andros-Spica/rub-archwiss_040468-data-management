...
### Example: Simple Data Science Project in R
...
2. **Data Cleaning**: Check for any missing values or potential outliers that might represent data entry errors.
   ...
   summary(df)
   ```
   Although `USArrests` doesn’t contain missing values, this step reminds us to handle such cases in real-world data.
...
3. **Data Exploration**: Visualize and summarize data to understand patterns. For instance, we might be interested in the value distribution of murder arrests.
   ...
   # Visualize the distribution of artifact "types" (represented by variables like 'Murder' here)
   boxplot(df$Murder, main="Distribution of 'Artifact Type A' Count by Region")
   ```
   This boxplot would give a quick view of the central tendencies and spread of murder arrests across the dataset.

4. **Modeling**: Create a simple model to analyze relationships between variables. For instance, let’s assume we’re exploring how `UrbanPop` (population percentage in urban areas) might relate to `Murder`.
   ```{r}
   model <- lm(Murder ~ UrbanPop, data = df)
   summary(model)
   ```
   This provides an understanding of how urban population percentage might be related to murder arrests in our dataset.

5. **Communication**: Present findings in an R Markdown document, incorporating both visualizations and model summaries. You can interpret results to suggest whether certain types of artifacts are more commonly found in highly populated regions.
...
* **Reshaping Data**  
Use `reshape()` to pivot data between wide and long formats.
...
* **Correlation**: Measure linear relationships between **numerical variables**.
...
```
In this case, the evidence is insufficient (`r cat(paste("p-value =", t.test(Weight ~ Blade.Sh, data = DartPoints_IandS)$p.value)``) for demonstrating that there is a consistent difference in weight between dart points with excurvate and straight blades.
