...
<!--- COMMENT 
consider here also the version using the base R pipeline operator:

data <- read.csv("data.csv") |>
  na.omit() |>
  subset(score > 10)

also: it is sufficient to write "eval = false" either in the header of the chode chunk or as chunk option. The latter is the quarto-way, the former the Rmarkdown-way but both work the same. To ensure future compatibility, I would suggest going for the quarto-way. 
...
<!-- COMMENT 
These kinds of test can also be implemented using base R: 
